...
test('formatKworkMessage не падает на спецсимволах Markdown в названии и причине (сообщение отправляется как plain text, без parse_mode)', () => {
  const text = formatKworkMessage({
  ...
  assert.match(text, /Доработка amo_crm и интеграция \[API\]/);
  assert.match(text, /сайт_v2 с формой \*заявки\* и интеграцией \[CRM\]/);
});
